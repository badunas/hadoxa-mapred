 ...
 */
public class AverageMarkJob {
    private final Configuration conf;
    private final Path inputPath;

    private Path tmpPath;

    public AverageMarkJob(Configuration conf, Path inputPath) {
        this.conf = conf;
        this.inputPath = inputPath;
    }

    public Job getJob() throws IOException {
        Job job = Job.getInstance(conf, "AverageMarkJob");
    ...
    public Path getTmpDirPath() throws IOException {
        if (tmpPath == null) {
            ...
            String tmpDir = "/tmp/" + getClass().getSimpleName() + "-marks-tmp";
...
//            fs.deleteOnExit(tmpPath);
    ...
    public static class JobReducer extends Reducer<Text,Text,Text,Text> {
        ...
        public void reduce(Text key, Iterable<Text> values, Context context) throws IOException, InterruptedException {
            ...
            int avg = sum / count;
            context.write(key, new Text("0 " + avg));
        }
    }
}
